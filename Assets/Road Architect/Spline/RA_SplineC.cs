...
#endif
#endregion
public class RA_SplineC : MonoBehaviour{
	#if UNITY_EDITOR
	public List<RA_SplineN> mNodes = new List<RA_SplineN>();
	public GameObject mSplineRoot;
	public RA_Road tRoad;
	public float distance = -1f;
	public Vector3[] CachedPoints;
	private const float CachedPointsSeperation = 1f;
	
	//Editor preview splines for add and insert:
	public RA_SplineF PreviewSpline;
	public RA_SplineI PreviewSplineInsert;
	...
	public RA_SplineC SpecialEndNode_Start_OtherSpline = null;
	public RA_SplineC SpecialEndNode_End_OtherSpline = null;
	...
	public void Setup_Trigger(){
		...
		if(!tRoad){ 
			if(mSplineRoot != null){
				tRoad = mSplineRoot.transform.parent.transform.gameObject.GetComponent<RA_Road>(); 
			}
		}
		if(tRoad){ 
			tRoad.UpdateRoad(); 
...
//		if(!Application.isEditor || (Application.isEditor && UnityEditor.EditorApplication.isPlaying)){ return; }
		...
		//Create spline nodes:
		RA_SplineN[] tNodesRaw = mSplineRoot.GetComponentsInChildren<RA_SplineN>();
		List<RA_SplineN> tList = new List<RA_SplineN>();
		...
		//tList.Sort(delegate(RA_Splin i1, Item i2) { return i1.name.CompareTo(i2.name); });
...
//			if(tRoad.bProfiling){ UnityEngine.Profiling.Profiler.EndSample(); }
		...
		if(PreviewSpline == null){
			PreviewSpline = mSplineRoot.AddComponent<RA_SplineF>();	
			PreviewSpline.RA_Spline = this;
		}
		//Setup preview spline for insertion mode:
		if(PreviewSplineInsert == null){
			PreviewSplineInsert = mSplineRoot.AddComponent<RA_SplineI>();
			PreviewSplineInsert.RA_Spline = this;
		}
		#endif
		
		int mNodesCount = mNodes.Count;
		RA_SplineN tNode = null;
        ...
        if(mNodesCount > 1){
            for(int i=0;i<mNodesCount;i++){
                ...
                if(tNode.bIsBridgeStart && !tNode.bIsTunnelStart){
					...
					if(fEnd > 0f || RA_RootUtil.IsApproximately(fEnd,0f,0.0001f)){
...
//		Vector3 tVect1 = default(Vector3);
	...
	private int CompareListByName(RA_SplineN i1, RA_SplineN i2){
		return i1.idOnSpline.CompareTo(i2.idOnSpline);
	}

	private void Setup_Nodes(ref RA_SplineN[] tNodesRaw){
		//Process nodes:
		int i=0;
		List<RA_SplineN> tNodes = new List<RA_SplineN>();
		int tNodesRawLength = tNodesRaw.Length;
...
//		float tNanCheck = 0f;
		...
		step = 0.5f / distance;
		RA_SplineN PrevNode = null;
		RA_SplineN ThisNode = null;
...
//		if(hi > RoadDefKeysArray.Length-1){ hi = RoadDefKeysArray.Length-1; }
	...
	private void GetClosestRoadDefValues(float tX, out float loF, out float hiF, out int loIndex, out int hiIndex) {
	    ...
	    while ((hi - lo) > 1) {
	        mid = Mathf.RoundToInt((lo + hi)/2);
	        if(RoadDefValuesArray[mid] < tX || RA_RootUtil.IsApproximately(RoadDefValuesArray[mid],tX,0.02f)){
	            lo = mid;
	        }else{
	            hi = mid;
	        }
		...
		if(RA_RootUtil.IsApproximately(RoadDefValuesArray[lo],tX,0.02f)){
			hi = lo; 
		}
...
//		if(RA_RootUtil.IsApproximately(f,0f,0.00001f)){
//			if(b){
...
//			}
//		}else 
//		if(RA_RootUtil.IsApproximately(f,1f,0.00001f) || f > 1f){
//			if(b){
...
//			}
//		}else{
		//RA_RootUtil.IsApproximately(f,1f,0.00001f)
...
//		}
		...
		param = RA_RootUtil.Ease(param, mNodes[idx].EaseIO.x, mNodes[idx].EaseIO.y);
...
//		if(RA_RootUtil.IsApproximately(f,1f,0.0001f)){
//			tVect1 = mNodes[mNodes.Count-1].pos;	
//			tVect2 = mNodes[mNodes.Count-1].tangent;
//			return;
//		}
//		else if(RA_RootUtil.IsApproximately(f,0f,0.0001f)){
//			tVect1 = mNodes[0].pos;	
...
//		}else{
		...
		if(idx < 0){ idx = 0; }
//			if(b && RA_RootUtil.IsApproximately(f,1f,0.00001f)){
//				idx = mNodes.Count-2;
//			}
//		}
	...
	public bool IsNearIntersection(ref Vector3 tPos, ref float nResult){
		...
		for(int i=0;i<mCount;i++){
			...
			if(tNode.bIsIntersection){
				tNode.RA_RI.Height = tNode.pos.y;
...
//				}
				...
				if(tNode.RA_RI.rType == RA_RoadIntersection.RoadTypeEnum.NoTurnLane){
					if(bUseSQ){
...
//					}
				}else if(tNode.RA_RI.rType == RA_RoadIntersection.RoadTypeEnum.TurnLane){
					if(bUseSQ){
...
//					}
				}else if(tNode.RA_RI.rType == RA_RoadIntersection.RoadTypeEnum.BothTurnLanes){
					if(bUseSQ){
...
//					}
	...
	public float IntersectionStrength(ref Vector3 tPos, ref float nResult, ref RA_RoadIntersection tInter, ref bool bIsPast, ref float p, ref RA_SplineN fNode){
		int mCount = GetNodeCount();
		float tDist;
		RA_SplineN tNode;
...
//					}
				...
				if(tDist <= MetersToCheck){
					if(tNode.RA_RI.bSameSpline){
						if(tNode.RA_RI.Node1.UID != tNode.UID){
							xNode = tNode.RA_RI.Node1;
						}else{
							xNode = tNode.RA_RI.Node2;
						}
	...
	public float IntersectionStrength_Next(Vector3 tPos){
		float nResult = 0f;
		RA_RoadIntersection tInter = null;
		bool bIsPast = false;
		float p = 0f;
		RA_SplineN fNode = null;
		return IntersectionStrength(ref tPos, ref nResult, ref tInter, ref bIsPast, ref p, ref fNode);
	}
	
	public bool IntersectionIsPast(ref float p, ref RA_SplineN oNode){
//		int mCount = GetNodeCount();
//		bool bIsPast;
//		RA_SplineN tNode = null;
//		for(int i=0;i<mCount;i++){
//			tNode = mNodes[i];
//			if(tNode.bIsIntersection){
//				float P1 = tNode.RA_RI.Node1.tTime - p; if(P1 < 0f){ P1 *= -1f; }
//				float P2 = tNode.RA_RI.Node2.tTime - p; if(P2 < 0f){ P2 *= -1f; }
//				
//				if(P1 > P2){
//					if(p > tNode.RA_RI.Node2.tTime){
//						bIsPast = true;	
//					}else{
//						bIsPast = false;	
//					}
//				}else{
//					if(p > tNode.RA_RI.Node1.tTime){
//						bIsPast = true;	
//					}else{
//						bIsPast = false;	
//					}
...
//		return false;
	...
	void DestroyIntersection(RA_SplineN tNode){
		if(tNode != null){
			if(tNode.bIsEndPoint){
				if(tNode.idOnSpline == 1 && tNode.RA_Spline.mNodes[0].bSpecialEndNode_IsStart){
					Object.DestroyImmediate(tNode.RA_Spline.mNodes[0].transform.gameObject);
					tNode.RA_Spline.bSpecialStartControlNode = false;
				}else if(tNode.idOnSpline == tNode.RA_Spline.GetNodeCount()-2 && tNode.RA_Spline.mNodes[tNode.RA_Spline.GetNodeCount()-1].bSpecialEndNode_IsEnd){
					Object.DestroyImmediate(tNode.RA_Spline.mNodes[tNode.RA_Spline.GetNodeCount()-1].transform.gameObject);
					tNode.RA_Spline.bSpecialEndControlNode = false;
				}
		...
		if(tNode.Intersection_OtherNode != null){
			if(tNode.Intersection_OtherNode.bIsEndPoint){
				if(tNode.Intersection_OtherNode.idOnSpline == 1 && tNode.Intersection_OtherNode.RA_Spline.mNodes[0].bSpecialEndNode_IsStart){
					Object.DestroyImmediate(tNode.Intersection_OtherNode.RA_Spline.mNodes[0].transform.gameObject);
					tNode.Intersection_OtherNode.RA_Spline.bSpecialStartControlNode = false;
				}else if(tNode.Intersection_OtherNode.idOnSpline == tNode.Intersection_OtherNode.RA_Spline.GetNodeCount()-2 && tNode.Intersection_OtherNode.RA_Spline.mNodes[tNode.Intersection_OtherNode.RA_Spline.GetNodeCount()-1].bSpecialEndNode_IsEnd){
					Object.DestroyImmediate(tNode.Intersection_OtherNode.RA_Spline.mNodes[tNode.Intersection_OtherNode.RA_Spline.GetNodeCount()-1].transform.gameObject);
					tNode.Intersection_OtherNode.RA_Spline.bSpecialEndControlNode = false;
				}
			}
		...
		if(tNode != null && tNode.Intersection_OtherNode != null){
			if(tNode.RA_Spline != tNode.Intersection_OtherNode.RA_Spline){
				if(tNode != null){ tNode.RA_Spline.tRoad.bUpdateSpline = true; }
				if(tNode.Intersection_OtherNode != null){ tNode.Intersection_OtherNode.RA_Spline.tRoad.bUpdateSpline = true; }
			}else{
				tNode.RA_Spline.tRoad.bUpdateSpline = true;
			}
		}else if(tNode != null){
			tNode.RA_Spline.tRoad.bUpdateSpline = true;
		}
	...
	public bool IsInBridge(float p){
		...
		for(int i=0;i<cCount;i++){
			KVP = BridgeParams[i];
			if(RA_RootUtil.IsApproximately(KVP.Key,p,0.0001f) || RA_RootUtil.IsApproximately(KVP.Value,p,0.0001f)){
				return true;
			}
	...
	public bool IsInBridgeTerrain(float p){
		...
		for(int i=0;i<cCount;i++){
			KVP = BridgeParams[i];
			if(RA_RootUtil.IsApproximately(KVP.Key+(10f/distance),p,0.0001f) || RA_RootUtil.IsApproximately(KVP.Value-(10f/distance),p,0.0001f)){
				return true;
			}
	...
	public void ActivateEndNodeConnection(RA_SplineN tNode1, RA_SplineN tNode2){
		ActivateEndNodeConnection_Do(tNode1,tNode2);
	}
	private void ActivateEndNodeConnection_Do(RA_SplineN tNode1, RA_SplineN tNode2){
		RA_SplineC xSpline = tNode2.RA_Spline;
...
//			bSecondNode_End = true;
		...
		RA_SplineN NodeCreated1 = null;
		RA_SplineN NodeCreated2 = null;
		...
		if(bFirstNode_Start){
			...
			if(mNodes[0].bSpecialEndNode){
				...
				RA_.Roads.RA_Construction.InsertNode(tRoad,true,tNode1_ExtraPos,false,0,true);
			}
			NodeCreated1 = mNodes[0];
		}else{
			bSpecialEndControlNode = true;
            RA_SplineN zNode1 = xSpline.GetLastNode_All();
            if (zNode1 != null && zNode1.bSpecialEndNode) {
                zNode1.transform.position = tNode1_ExtraPos;
                zNode1.pos = tNode1_ExtraPos;
			}else{
				RA_.Roads.RA_Construction.CreateNode(tRoad,true,tNode1_ExtraPos);	
			}
		...
		if(bSecondNode_Start){
			...
			if(xSpline.mNodes[0].bSpecialEndNode){
				...
				RA_.Roads.RA_Construction.InsertNode(xSpline.tRoad,true,tNode2_ExtraPos,false,0,true);
			}
			NodeCreated2 = xSpline.mNodes[0];
		}else{
			xSpline.bSpecialEndControlNode = true;
            RA_SplineN zNode2 = xSpline.GetLastNode_All();
            if (zNode2 != null && zNode2.bSpecialEndNode) {
                zNode2.transform.position = tNode2_ExtraPos;
                zNode2.pos = tNode2_ExtraPos;
			}else{
				RA_.Roads.RA_Construction.CreateNode(xSpline.tRoad,true,tNode2_ExtraPos);	
			}
		...
		float lWidth1 = tNode1.RA_Spline.tRoad.opt_LaneWidth;
		float lWidth2 = tNode2.RA_Spline.tRoad.opt_LaneWidth;
		float xWidth = Mathf.Max(lWidth1,lWidth2);
		
		float tDelay = 0f;
		if(tNode1.RA_Spline.tRoad.opt_Lanes > tNode2.RA_Spline.tRoad.opt_Lanes){
			tNode2.bSpecialRoadConnPrimary = true;
			NodeCreated2.bSpecialRoadConnPrimary = true;
			if(tNode2.RA_Spline.tRoad.opt_Lanes == 4){ xWidth *= 2f; }
			tDelay = (tNode1.RA_Spline.tRoad.opt_Lanes - tNode2.RA_Spline.tRoad.opt_Lanes) * xWidth;
			if(tDelay < 10f){ tDelay = 10f; }
			if(bSecondNode_Start){
				tNode2.RA_Spline.bSpecialEndNode_IsStart_Delay = true;
				tNode2.RA_Spline.SpecialEndNodeDelay_Start = tDelay;
				tNode2.RA_Spline.SpecialEndNodeDelay_Start_Result = tNode1.RA_Spline.tRoad.RoadWidth();
				tNode2.RA_Spline.SpecialEndNode_Start_OtherSpline = tNode1.RA_Spline;
			}else{
				tNode2.RA_Spline.bSpecialEndNode_IsEnd_Delay = true;
				tNode2.RA_Spline.SpecialEndNodeDelay_End = tDelay;
				tNode2.RA_Spline.SpecialEndNodeDelay_End_Result = tNode1.RA_Spline.tRoad.RoadWidth();
				tNode2.RA_Spline.SpecialEndNode_End_OtherSpline = tNode1.RA_Spline;
			}
		}else if(tNode2.RA_Spline.tRoad.opt_Lanes > tNode1.RA_Spline.tRoad.opt_Lanes){
			tNode1.bSpecialRoadConnPrimary = true;
			NodeCreated1.bSpecialRoadConnPrimary = true;
			if(tNode1.RA_Spline.tRoad.opt_Lanes == 4){ xWidth *= 2f; }
			tDelay = (tNode2.RA_Spline.tRoad.opt_Lanes - tNode1.RA_Spline.tRoad.opt_Lanes) * xWidth;
			if(tDelay < 10f){ tDelay = 10f; }
			if(bFirstNode_Start){
				tNode1.RA_Spline.bSpecialEndNode_IsStart_Delay = true;
				tNode1.RA_Spline.SpecialEndNodeDelay_Start = tDelay;
				tNode1.RA_Spline.SpecialEndNodeDelay_Start_Result = tNode2.RA_Spline.tRoad.RoadWidth();
				tNode1.RA_Spline.SpecialEndNode_Start_OtherSpline = tNode2.RA_Spline;
			}else{
				tNode1.RA_Spline.bSpecialEndNode_IsEnd_Delay = true;
				tNode1.RA_Spline.SpecialEndNodeDelay_End = tDelay;
				tNode1.RA_Spline.SpecialEndNodeDelay_End_Result = tNode2.RA_Spline.tRoad.RoadWidth();
				tNode1.RA_Spline.SpecialEndNode_End_OtherSpline = tNode2.RA_Spline;
			}
		}else{
			tNode1.bSpecialRoadConnPrimary = true;
			NodeCreated1.bSpecialRoadConnPrimary = true;
			tDelay = 0f;
			tNode1.RA_Spline.bSpecialEndNode_IsEnd_Delay = false;
			tNode1.RA_Spline.bSpecialEndNode_IsStart_Delay = false;
			tNode1.RA_Spline.SpecialEndNodeDelay_End = tDelay;
			tNode1.RA_Spline.SpecialEndNodeDelay_End_Result = tNode2.RA_Spline.tRoad.RoadWidth();
			tNode1.RA_Spline.SpecialEndNode_End_OtherSpline = tNode2.RA_Spline;
			tNode2.RA_Spline.bSpecialEndNode_IsEnd_Delay = false;
			tNode2.RA_Spline.bSpecialEndNode_IsStart_Delay = false;
			tNode2.RA_Spline.SpecialEndNodeDelay_End = tDelay;
			tNode2.RA_Spline.SpecialEndNodeDelay_End_Result = tNode1.RA_Spline.tRoad.RoadWidth();
			tNode2.RA_Spline.SpecialEndNode_End_OtherSpline = tNode1.RA_Spline;
		}
...
//		tNode1.RA_Spline.Setup_Trigger();
//		if(tNode1.RA_Spline != tNode2.RA_Spline){
//			tNode2.RA_Spline.Setup_Trigger();
//		}
		
		if(tNode1 != null && tNode2 != null){
			if(tNode1.RA_Spline != tNode2.RA_Spline){
				tNode1.RA_Spline.tRoad.PiggyBacks = new RA_SplineC[1];
				tNode1.RA_Spline.tRoad.PiggyBacks[0] = tNode2.RA_Spline;
			}
			tNode1.RA_Spline.tRoad.EditorUpdateMe = true;
		}
	}
	#endregion
	...
	public int GetNodeCountNonNull(){
		...
		for(int i=0;i<mCount;i++){
			if(mNodes[i] != null){
				tCount+=1;	
				if(mNodes[i].bIsIntersection && mNodes[i].RA_RI == null){
	...
	public RA_SplineN GetCurrentNode(float p){
		int mCount = GetNodeCount();
		RA_SplineN tNode = null;
		for(int i=0;i<mCount;i++){
			tNode = mNodes[i];
	...
	public RA_SplineN GetLastLegitimateNode(){
		int mCount = GetNodeCount();
		RA_SplineN tNode = null;
    ...
    public RA_SplineN GetLastNode_All() {
        int mCount = GetNodeCount();
	...
	public RA_SplineN GetPrevLegitimateNode(int tIndex){
		try{
			RA_SplineN tNode = null;
	...
	public RA_SplineN GetNextLegitimateNode(int tIndex){
		RA_SplineN tNode = null;
		int mCount = GetNodeCount();
