...
#endif
#endregion
public class RA_SplineF : MonoBehaviour{
	#if UNITY_EDITOR
	
	public class RA_SplineFN{
		public Vector3 pos;
	...
	public int tCount = 0;
	public List<RA_SplineFN> mNodes = new List<RA_SplineFN> ();
	...
	private void Setup_Nodes(ref Vector3[] tVects){
		...
		mNodes = new List<RA_SplineFN>();
		RA_SplineFN tNode;
		for(i=0;i<tVects.Length;i++){
			tNode = new RA_SplineFN();
			tNode.pos = tVects[i];
			mNodes.Add (tNode);
		}
