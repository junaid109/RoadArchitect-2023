...
using RA_.Roads.Splination;
using RA_.Roads.EdgeObjects;
using RA_;
#endif
public class RA_SplineN : MonoBehaviour{
	#if UNITY_EDITOR
	...
	public RA_SplineN SpecialNodeCounterpart = null;
	public RA_SplineN SpecialNodeCounterpart_Master = null;
	public RA_SplineN SpecialNodeCounterpart_Old = null;
	...
	public RA_SplineC RA_Spline;
	public bool bDestroyed = false;
	public string UID; //Unique ID
	public RA_SplineN Intersection_OtherNode;
	...
	public List<RA_SplineN> node_connected;
	...
	public RA_SplineN TunnelCounterpartNode = null;
	...
	public RA_SplineN BridgeCounterpartNode = null;

	public RA_RoadIntersection RA_RI = null;
	
	public RA_.Roads.RA_Intersections.iConstructionMaker iConstruction;
	...
	public EdgeObjectMaker AddEdgeObject(){
		...
		EOM.tNode = this;
		EOM.SetDefaultTimes(bIsEndPoint,tTime,NextTime,idOnSpline,RA_Spline.distance);
		EOM.StartPos = RA_Spline.GetSplineValue(EOM.StartTime);
		EOM.EndPos = RA_Spline.GetSplineValue(EOM.EndTime);
		EdgeObjects.Add(EOM);
		return EOM;
	}
	...
	public SplinatedMeshMaker AddSplinatedObject(){
		...
		SMM.Init(RA_Spline,this,transform);
		SplinatedObjects.Add(SMM);
		SMM.SetDefaultTimes(bIsEndPoint,tTime,NextTime,idOnSpline,RA_Spline.distance);
		SMM.StartPos = RA_Spline.GetSplineValue(SMM.StartTime);
		SMM.EndPos = RA_Spline.GetSplineValue(SMM.EndTime);
		return SMM;
	}
	...
	public void LoadWizardObjectsFromLibrary(string tFileName, bool _bIsDefault, bool _bIsBridge){
		...
		RA_.Roads.RA_RoadUtil.LoadNodeObjects(tFileName,this,_bIsDefault,_bIsBridge);
	}
...
//			RA_.Roads.RA_RoadUtil.Construction3DTri tTri = null;
...
////			
////			tRect = new RA_.Roads.RA_RoadUtil.Construction3DRect(P1,P2,P3,P4);
////
////			Gizmos.color = new Color(0f,0f,1f,1f);
...
//	}
//	
	public List<RA_.Roads.RA_RoadUtil.Construction3DTri> tTriList;
	public List<Vector3> tHMList;
	public bool bGizmoDrawIntersectionHighlight = false;
	
	void OnDrawGizmos (){ 
		if(RA_Spline == null){ return; }
		if(RA_Spline.tRoad == null){ return; }
		if(!RA_Spline.tRoad.opt_GizmosEnabled){ return; }
		if(bIgnore){ return; }
...
//		}
        ...
        if (!bIsEndPoint && !bSpecialEndNode && mCount > 1 && ((idOnSpline+1) < RA_Spline.mNodes.Count))
        {
			P1 = pos;
			P2 = RA_Spline.mNodes[idOnSpline+1].pos;
...
//		float CurrentGrade = CurrentYDiff/CurrentDistance;
		...
		MaxY = (RA_Spline.tRoad.opt_MaxGrade * CurrentDistance) + pos.y;
		MinY = pos.y - (RA_Spline.tRoad.opt_MaxGrade * CurrentDistance);
...
//			DifferenceFromMax = MinimumY - tPos.y;
	...
	public void EnsureGradeValidity(int iStart = -1, bool bIsAddToEnd = false){
		...
		RA_SplineN NextNode = null;
		
		if(bIsAddToEnd && RA_Spline.GetNodeCount() > 0){
			PrevNode = RA_Spline.mNodes[RA_Spline.GetNodeCount()-1];
		}else{
			if(iStart == -1){
				PrevNode = RA_Spline.GetPrevLegitimateNode(idOnSpline);
			}else{
				PrevNode = RA_Spline.GetPrevLegitimateNode(iStart);
			}
		}
	...
	public void ResetNavigationData(){
		...
		node_connected = null;
		node_connected = new List<RA_SplineN>();
	}
	
	public void BreakConnection(){
		RA_SplineN tNode2 = SpecialNodeCounterpart;
				
		if(bSpecialEndNode_IsStart){
			RA_Spline.bSpecialStartControlNode = false;
			RA_Spline.bSpecialEndNode_IsStart_Delay = false;
		}else if(bSpecialEndNode_IsEnd){
			RA_Spline.bSpecialEndControlNode = false;
			RA_Spline.bSpecialEndNode_IsEnd_Delay = false;
		}
		if(tNode2.bSpecialEndNode_IsStart){
			tNode2.RA_Spline.bSpecialStartControlNode = false;
			tNode2.RA_Spline.bSpecialEndNode_IsStart_Delay = false;
		}else if(tNode2.bSpecialEndNode_IsEnd){
			tNode2.RA_Spline.bSpecialEndControlNode = false;
	...
	public void SetupSplinationLimits(){
		...
		MaxSplination = tTime;
		int mCount = RA_Spline.GetNodeCount();
	...
	public bool CanBridgeStart(){
		...
		if(idOnSpline > 0){
			if(RA_Spline.mNodes[idOnSpline-1].bIsBridgeStart){
				return false;	
			}
		}
		...
		if(idOnSpline < (mCount-1)){
			...
			if(RA_Spline.bSpecialEndControlNode){
	...
	public bool CanBridgeEnd(){
		...
		if(idOnSpline < (mCount-1)){
			if(RA_Spline.bSpecialStartControlNode){
				if(idOnSpline == 2){
					return false;	
				}
			}else{
		...
		for(int i=idOnSpline;i>=0;i--){
			if(RA_Spline.mNodes[i].bIsBridgeStart){
				if(!RA_Spline.mNodes[i].bIsBridgeMatched){
					return true;
				}else{
					return false;	
				}
	...
	public bool CanTunnelStart(){
		...
		if(idOnSpline > 0){
			if(RA_Spline.mNodes[idOnSpline-1].bIsTunnelStart){
				return false;	
			}
		}
	...
	public bool CanTunnelEnd(){
		...
		for(int i=idOnSpline;i>=0;i--){
			if(RA_Spline.mNodes[i].bIsTunnelStart){
				if(!RA_Spline.mNodes[i].bIsTunnelMatched){
					return true;
				}else{
					return false;	
				}
	...
	public bool IsStraight(){
		...
		if(id1 > -1 && id1 < mCount){
			if(!IsApproxTwoThirds(ref pos,RA_Spline.mNodes[id1].pos)){
				return false;
			}
		}
		
		if(id2 > -1 && id2 < mCount){
			if(!IsApproxTwoThirds(ref pos,RA_Spline.mNodes[id2].pos)){
				return false;
			}
		}
		
		if(id3 > -1 && id3 < mCount){
			if(!IsApproxTwoThirds(ref pos,RA_Spline.mNodes[id3].pos)){
				return false;
			}
		}
	...
	private static bool IsApproxTwoThirds(ref Vector3 V1, Vector3 V2){
		int cCount = 0;
		if(RA_RootUtil.IsApproximately(V1.x,V2.x,0.02f)){
			cCount+=1;
		}
		if(RA_RootUtil.IsApproximately(V1.y,V2.y,0.02f)){
			cCount+=1;
		}
		if(RA_RootUtil.IsApproximately(V1.z,V2.z,0.02f)){
			cCount+=1;
		}
